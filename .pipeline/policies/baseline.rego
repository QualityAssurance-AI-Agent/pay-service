...
allow {
	...
	gate_load_test_before_production
...
stage_production_exists {
	some s
	input.stages[s].is_production
}

stage_production_at_or_before(i) {
	some s
	input.stages[s].is_production
	s <= i
}
...
# Rule: a passing 'load-test' GATE must precede every production stage when the change touches services/**
gate_load_test_before_production {
	not stage_production_exists
}

gate_load_test_before_production {
	not touched_load_test_production
}
...
gate_load_test_before_production {
	...
	not stage_production_at_or_before(i)
}

touched_load_test_production {
	some k
...
deny[msg] {
	not gate_load_test_before_production
	msg := sprintf("ORDERING GATE VIOLATION: the change touches services/**, so a 'load-test' gate must run and pass before any production stage", [])
}
